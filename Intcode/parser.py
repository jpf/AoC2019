    ...
    def explain(self):
        ...
        for i in parsed:
            ...
            if i["type"] == "Parameter" and i["mode"] == "Immediate":
                type_hint = "="
            elif i["type"] == "Parameter" and i["mode"] == "Positional":
                type_hint = "~"
                extra = "({})".format(self.tokens[i["value"]]["value"])
            elif i["type"] == "Unknown":
                type_hint = "?"
            if "kind" in i and i["kind"] == "Output":
                extra = "=> [{}]".format(self.tokens[i["value"]]["value"])
            i["type_hint"] = type_hint
            i["extra"] = extra
                
            out = "{address: >3} {value: >6} {type_hint} {name: <10} {extra}".format(**i)
...
# cpu.post()
cpu.reset()
# cpu.memory = list(immediate_mode_eq_eight)
# cpu.memory = list(TEST)
cpu.memory = list(position_mode_jmp_test)
cpu.explain()
