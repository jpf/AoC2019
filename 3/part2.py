...
def make_wire(steps):
    ...
    for step in steps.split(","):
        direction = step[0]
        length = int(step[1:]) + 1
        # print(f"dir: {direction} len: {length}")

        to_x = [from_x]
        to_y = [from_y]

        if direction == "R":
            to_x = range(from_x, from_x + length)
        elif direction == "L":
            to_x = range(from_x, from_x - length, -1)
        elif direction == "U":
            to_y = range(from_y, from_y + length)
        elif direction == "D":
            to_y = range(from_y, from_y - length, -1)

        # print(f"{step} => x: {to_x} y: {to_y}")
        for x in to_x:
            for y in to_y:
                if len(wire) > 0 and wire[-1:][0] == (x, y):
                    pass
                else:
                    wire.append((x, y))
                    from_x, from_y = x, y
...
def find_min_steps(wires):
    a, a_count = make_wire(wires[0])
    b, b_count = make_wire(wires[1])

    aa = set(a)
    bb = set(b)
    coords = list(filter(lambda x: x != (0, 0), aa.intersection(bb)))
    # print(coords)
    found = []
    for coord in coords:
        steps = 0
        ...
        for index, value in enumerate(b):
            if value == coord:
                steps += index
                break
        # print(f"{coord} is {steps} steps")
        found.append(steps)
    # found = list(filter(lambda x: x, map(lambda x: abs(x[0]) + abs(x[1]), coords)))
    # print(found)
    ans = min(found)
    return ans

import sys
for vector in test_vectors_two:
    want = vector['answer']
    got = find_min_steps(vector['wires'])
    assert want == got
    # print(f"Want: {want}")
    # print(f"Got:  {got}")

answer = find_min_steps(data['wires'])
